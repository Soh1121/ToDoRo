resource "aws_elasticache_replication_group" "this" {
  ...
  // Redis settings
  replication_group_id          = "${local.service_name}-${local.env_name}-${local.service_name}"
  replication_group_description = "Session storage for ToDoRo"
  engine_version                = "6.x"
  port                          = 6379
  ...
  // Security
  security_group_ids = [
    data.terraform_remote_state.network_main.outputs.security_group_cache_todoro_id
  ]
